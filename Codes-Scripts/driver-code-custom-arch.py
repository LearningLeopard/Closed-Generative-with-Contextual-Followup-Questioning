# %%
import torch
from transformers import AutoModelWithLMHead, AutoTokenizer, T5Tokenizer, T5ForConditionalGeneration, AutoConfig, AutoModel
...
def generate_qa_model_answer(question, meta_tags, context=""):
    ...
    output = qa_model.generate(**encoded_input, max_length=512)
    output = qa_model_tokenizer.decode(output[0], skip_special_tokens=True)
    return "".join(output)

# %%
def get_confidence_level(question, answer):
    final_input_string = f"{question} [SEP] {answer}"
    tokenized_inputs = correctness_tokenizer(final_input_string, truncation=True, add_special_tokens=True, return_tensors='pt')
...
def generate_answer(row):
    ...
    while steps < MAX_STEPS and confidence < CONFIDENCE_THRESHOLD:
        ...
        steps += 1
    return {"No of Ans": steps, "Predicted Answer": answers_history[-1], "All Answers": answers_history}

# %%
prediction = visa_qa_testing.map(generate_answer)
...
prediction.save_to_disk("../Datasets/Visa_prediction_FineTuned_Custon_Arch_1000")
